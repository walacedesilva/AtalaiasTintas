...
```
Error: locator.fill: Error: strict mode violation: getByLabel('Senha') resolved to 2 elements:
    1) <input id="password" name="password" type="password" aria-invalid="false" placeholder="Digite sua senha" autocomplete="current-password" class="form-input pl-10 pr-10 "/> aka getByRole('textbox', { name: 'Senha' })
    2) <button type="button" aria-label="Mostrar senha" class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 transition-colors">…</button> aka getByRole('button', { name: 'Mostrar senha' })
...
```ts
  ...
  25  |     await expect(page.getByLabel('Senha')).toBeVisible();
  ...
  46  |     await page.getByLabel('Senha').fill('123');
...
> 86  |     await page.getByLabel('Senha').fill('wrongpassword123');
      |                                    ^ Error: locator.fill: Error: strict mode violation: getByLabel('Senha') resolved to 2 elements:
  ...
  133 |     await page.getByLabel('Senha').fill('admin123');
  ...
  152 |     await expect(page.getByLabel('Senha')).toBeFocused();
  153 | 
  ...
  164 |     await page.getByLabel('Senha').fill('testpass123');
  ...
  177 |     await expect(page.getByText('Atalaia Tintas')).toBeVisible();
  178 |     await expect(page.getByLabel('Nome de usuário')).toBeVisible();
  179 |     await expect(page.getByLabel('Senha')).toBeVisible();
  ...
  184 |     await page.getByLabel('Senha').fill('password123');
  185 |     
