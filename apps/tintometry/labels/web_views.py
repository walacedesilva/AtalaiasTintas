...
from apps.tintometry.models import (
    MisturaTinta, FormulaTintometrica, PigmentoUniversal,
    CorDefinida, BaseProduct
)
...
def dashboard_view(request):
    ...
    try:
        ...
        context = {
            ...
            'custo_total_mes': stats_volume.get('custo_total', 0) or 0,
...
def nova_mistura_view(request):
    ...
    if request.method == 'POST':
        try:
            ...
            observacoes_cliente = request.POST.get('observacoes_cliente', '').strip()
            observacoes_internas = request.POST.get('observacoes_internas', '').strip()
            ...
            # Calcular itens da fórmula
            mistura.calcular_quantidades_pigmentos()
    ...
    try:
        ...
        formulas = FormulaTintometrica.objects.filter(
            is_active=True
        ).select_related('cor_definida', 'base_produto').order_by(
...
def search_formulas(request):
    ...
    try:
        ...
        formulas = FormulaTintometrica.objects.filter(
            ...
            is_active=True
        ).select_related('cor_definida', 'base_produto')[:10]
        ...
        for formula in formulas:
            results.append({
                ...
                'base_produto': formula.base_produto.nome_variacao,
...
def update_mistura_status(request, mistura_id):
    ...
    try:
        ...
        if new_status not in dict(MisturaTinta.SITUACAO_CHOICES):
            return JsonResponse({
                'success': False,
